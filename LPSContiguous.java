...
public class LPSContiguous {
	...
	public static HashSet<String> lps(String seq){ 
		int n = seq.length(); 
		int j; 
		...
		for (int cl=2; cl<=n; cl++){ 
			for (int i=0; i<n-cl+1; i++){ 
				...
				if (seq.charAt(i) == seq.charAt(j) && cl == 2) { 
					...
					String x = seq.substring(i,j+1); 
					
					if(isPlaindrome(x)==true) {
						L[i][j] = L[i+1][j-1] + 2; 
		...
		for(int a=0;a<K.length;a++) {
			for(int b=a;b<K.length;b++) {
				if(K[a][b].length()==ans.length()) {
					set.add(K[a][b]); 
				}
			}
		}
